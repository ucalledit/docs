...
<Steps>
  ...
  <Step title="On-Chain Escrow Execution">
    The system executes a 3-step Solana transaction flow. The oracle pays all gas fees so users and MMs only need USDC. User/MM signatures are obtained via Privy TEE wallets using `signTransaction` (sign-only), then the oracle co-signs and broadcasts locally.

    1. **create_parlay** — Oracle creates the escrow PDA and token account (oracle signs locally)
    2. **deposit_user** — User's USDC stake is transferred to the escrow (Privy TEE signs for user, oracle co-signs)
    3. **deposit_mm** — Market maker's USDC risk is transferred to the escrow (Privy TEE signs for MM, oracle co-signs)

    All three transaction signatures and the escrow account address are stored in the database and displayed in the UI with links to Solana Explorer.

    <Card title="Payment Architecture" icon="link" href="/guides/payment-architecture">
      Learn more about the Privy TEE wallet integration, oracle-sponsored gas, and the escrow program.
    </Card>
  ...
  <Step title="Settlement">
    A background service polls Kalshi every 30 seconds to check if all leg markets have settled. Settlement is winner-take-all:

    - **All legs win** — The entire escrow balance (user_stake + mm_risk) is transferred to the user (`settled_win`)
    - **Any leg loses** — The entire escrow balance is transferred to the market maker (`settled_loss`)
  </Step>
</Steps>
